...
One challenge with set operations is that the number of features that are generates has no mechanical relationship to the number of features that were passed to the set function. As a result, we can't just use the tricks we used for basic functions like `cbind`ing the results back into the original data frame, because they may be of entirely different lengths. 
...
districts@data$id2 <- rownames(districts@data)
districts@data <- join(districts@data, result, by="id2", type="left")
```

Now we can calculate the share of each district within 7km!

```{r}
districts@data$result / districts@data$Shape_Area
```


## 3.2 Last Word: `byid` Issues
...
#. Measure the share of each district within 7km of *any* drone strike using the `gArea` command for each district, and for the buffered strikes in each district. **When you merge the areas of your intersections back in with districts, be sure to look at that table -- why are there missing values? Do you need to worry abou them?**
...
fata@data$id2 <- rownames(fata@data)

fata@data <- join(fata@data, result, by="id2", type="left")
...
fata@data[is.na(fata@data$area_near_strikes),"area_near_strikes"]<-0
fata@data$share_covered <- fata@data$area_near_strikes / fata@data$Shape_Area

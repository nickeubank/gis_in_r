...
#. What is the CRS of `sf`?   What is the CRS of `sf_ft`?

#. Reproject `sf.bp` so it matches the projection of `sf` and assign it to a new object called `sf.ft.newproj`.
...
![different projections of the same points](images/sf_ft_projs.png)
...
For the next exercise we want to count all the block parties that occur *within* each census tract in San Francisco. To achieve this this we join the points of block parties to the census tract polygon. You might be familiar with this operation from other GIS packages.
...
#. Our points are _sf.bp.newproj_ and our polygons are _sf_. The points file has many attribute data (check with `names()`), and we only really need one of those columns to add it up to the count. We choose here _"locatnd"_, but you could choose any other one and it will give the same result. So in order to aggregate we say:
```
ft.count <- aggregate(x = sf.ft.newproj["locatnd"], by = sf, FUN = length)
```
...
![Philly Selection](images/birite.png)

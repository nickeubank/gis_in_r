...
If `SpatialGridDataFrame()` is given a DataFrame with more than one column where each column has as many rows as the grid has cells, it will treat each column as a different dataset. This means a single raster could have multiple forms of data associated with each cell, like average rainfall and average temperature. 

In a similar fashion to the example above we can use `readGDAL` from the `rgdal` package to read in a raster file. It will require one parameter as a minimum, which is the filename of the raster. So let's do this.

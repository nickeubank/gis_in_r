...
# Look at data
head(strikes.projected@data)

# Number per district
over.list <- over(pk.dist,geometry(strikes.projected),returnList=TRUE)
num.strikes <- sapply(over.list, length)
pk.dist@data <- cbind(pk.dist@data, num.strikes)
pk.dist@data[pk.dist@data$num.strikes != 0,]

# Avg Fatality
avg.fatality <- over(pk.dist,strikes.projected[c("Killed","CiviliansK")],fn=mean)
pk.dist@data <- cbind(pk.dist@data, avg.fatality)
pk.dist@data[!is.na(pk.dist@data$Killed),]
...
pk.dist.rasterproj@data <- cbind(pk.dist.rasterproj@data, pops.summed)

pk.dist.rasterproj@data$strikes.percap <- 
  pk.dist.rasterproj@data$num.strikes / pk.dist.rasterproj@data$pops.summed

head(pk.dist.rasterproj@data[pk.dist.rasterproj@data$num.strikes >0,])
```
